...
export interface PagedResponse<T> {
  data: T[]; total: number; current_page: number; per_page: number; last_page: number;
}
...
export class ApiService {
  ...
  get<T>(path: string, params?: QueryParams): Observable<T> {
    ...
    return this.http.get<T>(`${this.baseUrl}/${path}`, { params: this.buildParams(params) })
      .pipe(catchError(err => {
        // On 503 (cold start), retry after 5 seconds
        if (err.status === 503) {
          return timer(5000).pipe(
            switchMap(() => this.http.get<T>(`${this.baseUrl}/${path}`, { params: this.buildParams(params) })),
